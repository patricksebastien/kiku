...
#define PLUGIN_TITLE "PulseAudio & libpd (monitor) A/D-in plugin for Julius"
#define INPUT_OPT "pulseaudio-libpd-monitor"
...
{	
	...
	// output
	if (!(s = pa_simple_new(NULL, "kiku", PA_STREAM_PLAYBACK, NULL, "voice recognition", &ss, NULL, &ba, &errorpa))) {
		return FALSE;
	}
...
adin_close()
{
	...
	/*
	if (pa_simple_drain(s, &errorpa) < 0) {
